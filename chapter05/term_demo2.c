/******************************************************************************
  ...
  Purpose        : Allows user to control O_NONBLOCK, ICANON, VMIN, VTIME
                   ...
                   reads from the terminal. HANDLES SIGNALS.
  Usage          : 
                   term_demo2 [OPTIONS]
                   OPTIONS 
  ...
  Build with     : gcc -Wall -g -o term_demo2 term_demo2.c
...
/*
 * get_response simulates an interactiev loop in which a user is expected
 ...
 * 2. whether the file connection is in non-delay (non-blocking) mode or not;
 * 3. the number of timeouts to wait until it gives up, and
 * 4. the prompt string to display.
 * It displays the number of tries and the elapsed time as it waits.
 */
int  get_response( FILE* fp, ui_params up );

/*
 *  Waits until one of y,Y,n, or N is entered. If it returns without one of 
 *  these, it returns EOF
 */
int get_valid_char(FILE* fp);
...
int get_response( FILE* fp, ui_params  uip )
{
    ...
    while ( TRUE ){
        ...
        if ( !uip.isblocking )
             sleep(uip.sleeptime);				
        input = tolower(get_valid_char(fp));        	
            if ( 'y' == input  || 'n' == input ) {
            return input;
        }
        else {			
            BEEP;
            time(&time_now);
            printf( "\n%d seconds elapsed, %d tries left\n",
...
int get_valid_char(FILE* fp)
{
    int n;
    unsigned char c;
    while ( ( n = read(fileno(fp), &c, 1) ) > 0 && strchr("yYnN",c) == NULL)
        /* no -op */ ;  
    return c;
}
...
void save_restore_tty(int fd, int action, struct termios *copy)
{
    ...
    if ( RETRIEVE == action ){
        ...
        fprintf(stderr, "Illegal action to set_tty().\n");
}
/*****************************************************************************/
...
void interrupt_handler(int signum)
/*
 *  restores terminal if program is to be terminated early
 */
{   
